...
python game_qa_clone.py record scenarios\run.json --method raw --live
...
- `--method`: 녹화 방식입니다. 기본값은 `poll`입니다. 게임 실행 중 키보드만 안 잡히면 `raw`를 먼저 시도하세요. `hook`은 마우스 휠 입력까지 잡을 수 있지만 일부 게임에서 입력이 안 잡힐 수 있습니다.
...
- 게임 포커스 중 `poll` 방식에서 마우스는 잡히는데 키보드가 안 잡히면 `--method raw`를 사용하세요.
