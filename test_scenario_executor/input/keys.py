"""Keyboard and mouse key maps used by the input clone module."""
...
def require_windows() -> None:
    if sys.platform != "win32":
        raise RuntimeError("test_scenario_executor requires Windows for keyboard/mouse control")


def scan_code_for_vk(vk: int) -> int:
    """Return the hardware scan code for a VK code."""
    if sys.platform != "win32":
        return 0
    scan = int(ctypes.windll.user32.MapVirtualKeyW(vk, MAPVK_VK_TO_VSC))
    arrow_fallback = {0x25: 0x4B, 0x26: 0x48, 0x27: 0x4D, 0x28: 0x50}
    return scan or arrow_fallback.get(vk, 0)
